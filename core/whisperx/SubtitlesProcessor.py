...
class SubtitlesProcessor:
    def __init__(self, segments, lang, max_line_length=45, min_char_length_splitter=30, is_vtt=False):
        ...
        complex_script_languages = ['th', 'lo', 'my', 'km', 'am', 'ko', 'ja', 'zh',
                                    'ti', 'ta', 'te', 'kn', 'ml', 'hi', 'ne', 'mr', 'ar', 'fa', 'ur', 'ka']
        ...
        if has_prev_end:
            ...
            if has_next_start:
                ...
                if next_segment_start_time:
                    words[i]['end'] = next_segment_start_time if next_segment_start_time - \
                        words[i - 1]['end'] <= 1 else next_segment_start_time - 0.5
                else:
    ...
    def process_segments(self, advanced_splitting=True):
        ...
        for i, segment in enumerate(self.segments):
            next_segment_start_time = self.segments[i +
                                                    1]['start'] if i + 1 < len(self.segments) else None

            if advanced_splitting:

                split_points = self.determine_advanced_split_points(
                    segment, next_segment_start_time)
                subtitles.extend(self.generate_subtitles_from_split_points(
    ...
    def determine_advanced_split_points(self, segment, next_segment_start_time=None):
        ...
        total_char_count = sum(len(word['word']) if isinstance(
            word, dict) else len(word) + add_space for word in words)
        ...
        for i, word in enumerate(words):
            ...
            if char_count >= self.max_line_length:
                ...
                if char_count_before >= self.min_char_length_splitter:
                    ...
                    char_count = sum(len(words[j]['word']) if isinstance(words[j], dict) else len(
                        words[j]) + add_space for j in range(last_split_point, i + 1))
    ...
    def generate_subtitles_from_split_points(self, segment, split_points, next_start_time=None):
        ...
        for split_point in split_points:
            ...
            if isinstance(fragment_words[0], dict):
                ...
                next_start_time_for_word = words[split_point +
                                                 1]['start'] if split_point + 1 < len(words) else None
                ...
                current_duration = (current_word_count /
                                    total_word_count) * total_time
